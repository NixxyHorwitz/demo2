                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered">
                            ...
                            <tbody>
                                ...
                                $query_list = "SELECT login_logs.*, users.username FROM login_logs INNER JOIN users ON login_logs.user_id = users.id WHERE login_logs.id <> ''";
                                ...
                                if (!empty($q_search)) {
                                    $query_list .= " AND (users.username LIKE '%$q_search%' OR login_logs.ip_address LIKE '%$q_search%' OR login_logs.user_agent LIKE '%$q_search%')";
                                }

                                $query_list .= " ORDER BY login_logs.id DESC";
                                ...
                                while ($log = mysqli_fetch_assoc($new_query)) { ?>
                                    <tr>
                                        ...
                                        <td class="text-nowrap">
                                            <a href="javascript:;" onclick="modal_open('detail', 'md', '<?= base_url('babikode/user/detail?id=' . $log['user_id']); ?>');" class="btn btn-glow btn-sm btn-bg-gradient-x-purple-red"><?= $log['username']; ?></a>
                                        </td>
                                        <td class="text-nowrap"><a href="javascript:;" class="btn btn-glow btn-bg-gradient-x-purple-red btn-sm"><?= $log['ip_address']; ?></a></td>
                        ...
                        <div class="col-md-12">
                            <nav aria-label="Page navigation">
                                <ul class="pagination pagination-md justify-content-center">
                                    <?php
                                    // start paging link
                                    $url = base_url('babikode/login_log/');
...
<script type="text/javascript">
    $(function() {
        $('#table-row').on('change', function() {
            ...
            window.location = "<?= base_url('babikode/login_log/'); ?>?row=" + row + "&start_date=" + start_date + "&end_date=" + end_date + "&search=" + search;
        });
    });
