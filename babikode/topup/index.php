        ...
        <div class="breadcrumbs-top float-md-right">
            <div class="breadcrumb-wrapper mr-1">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active">Top Up
                    </li>
            ...
            <div class="card">
                ...
                <div class="card-body pt-0">
                    ...
                    <div class="btn-group flex-wrap">
                        <a href="<?= base_url('babikode/topup/'); ?>" class="btn btn-glow btn-primary <?= (empty($q_status) or $q_status == '') ? 'active' : ''; ?>">Semua</a>
                        <a href="<?= base_url('babikode/topup/?status=pending'); ?>" class="btn btn-glow btn-primary <?= ($q_status == 'pending') ? 'active' : ''; ?>">Pending</a>
                        <a href="<?= base_url('babikode/topup/?status=success'); ?>" class="btn btn-glow btn-primary <?= ($q_status == 'success') ? 'active' : ''; ?>">Success</a>
                        <a href="<?= base_url('babikode/topup/?status=Expired'); ?>" class="btn btn-glow btn-primary <?= ($q_status == 'Expired') ? 'active' : ''; ?>">Expired</a>
                    </div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered">
                            ...
                            <tbody>
                                ...
                                $query_list = "SELECT topups.*, users.phone FROM topups INNER JOIN users ON topups.user_id = users.id WHERE topups.id <> ''";

                                if (isset($q_status) && in_array($q_status, array('pending', 'success', 'canceled'))) {
                                    $query_list .= " AND topups.status = '" . ucfirst($q_status) . "'";
                                }

                                if (!empty($q_start_date) and !empty($q_end_date)) {
                                    $query_list .= " AND DATE(topups.created_at) BETWEEN '" . $q_start_date . "' AND '" . $q_end_date . "'";
                                } elseif (!empty($q_start_date) and empty($q_end_date)) {
                                    $query_list .= " AND DATE(topups.created_at) BETWEEN '" . $q_start_date . "' AND '" . $q_start_date . "'";
                                } elseif (empty($q_start_date) and !empty($q_end_date)) {
                                    $query_list .= " AND DATE(topups.created_at) BETWEEN '" . $q_end_date . "' AND '" . $q_end_date . "'";
                                }
...
if (!empty($q_search)) {
    $query_list .= " AND (
        topups.id LIKE '%$q_search%' 
        OR users.phone LIKE '%$q_search%'
    )";
}

                                $query_list .= " ORDER BY topups.id DESC";
                                ...
                                while ($tu = mysqli_fetch_assoc($new_query)) {
                                    $color = match ($tu['status']) {
                                        'Success' => 'blue-green disabled',
                                        'Expired' => 'red-pink disabled',
                                        default => 'orange-yellow'
                                    };
                                ?>
                                    <tr>
                                        <td><?= $tu['id']; ?></td>
                                        <td><?= format_date(date: substr($tu['created_at'], 0, -8), print_day: true, short_month: true) . ' ' . substr($tu['created_at'], 11, -3); ?></td>
                                        <td class="text-nowrap">
                                            <a href="javascript:;" onclick="modal_open('detail', 'md', '<?= base_url('babikode/user/detail?id=' . $tu['user_id']); ?>');" class="btn btn-glow btn-sm btn-bg-gradient-x-purple-red"><?= $tu['phone']; ?></a>
                                        </td>
                                        <td><?= $tu['method']; ?></td>
                                        <td><?= $tu['no_rek'] . ' (' . $tu['name_rek'] . ')'; ?></td>
                                        <td>Rp <?= number_format($tu['amount'], 0, ',', '.'); ?></td>
                                        <td>
                                            <div class="btn-group">
                                                <button type="button" class="btn btn-glow btn-sm btn-bg-gradient-x-<?= $color; ?>" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true"><?= $tu['status']; ?><i class="ft-chevron-down ml-1"></i></button>
                                                <div class="dropdown-menu">
                                                    <a class="dropdown-item" href="javascript:;" onclick="get_data('<?= base_url('babikode/topup/status?id=' . $tu['id'] . '&status=success'); ?>');">Success</a>
                                                    <a class="dropdown-item" href="javascript:;" onclick="get_data('<?= base_url('babikode/topup/status?id=' . $tu['id'] . '&status=Expired'); ?>');">Expired</a>
                                                </div>
                                            </div>
                                        </td>
                                        <td class="text-nowrap">
                                            <a href="javascript:;" onclick="swal_delete('<?= $tu['id']; ?>', '<?= base_url('babikode/topup/delete?id=' . $tu['id']); ?>');" class="btn btn-glow btn-sm btn-bg-gradient-x-red-pink"><b><i class="ft-trash mr-1"></i>Hapus</b></a>
                                        </td>
                                    </tr>
                                <?php
                                } ?>
    ...
    $(function() {
        $('#table-row').on('change', function() {
            ...
            if (status == '' || status == undefined) {
                window.location = "<?= base_url('babikode/topup/'); ?>?row=" + row + "&start_date=" + start_date + "&end_date=" + end_date + "&search=" + search;
            } else {
                window.location = "<?= base_url('babikode/topup/'); ?>?status=" + status + "&row=" + row + "&start_date=" + start_date + "&end_date=" + end_date + "&search=" + search;
            }
        });
    });
