        ...
        <div class="col-md-12">
            <div class="card">
                ...
                <div class="card-body pt-0">
                    ...
                    $q_type = (isset($_GET['type'])) ? protect($_GET['type']) : '';
                    ...
                    <div class="btn-group flex-wrap">
                        <a href="<?= base_url('babikode/point_log'); ?>" class="btn btn-glow btn-primary <?= (empty($q_type) or $q_type == '') ? 'active' : ''; ?>">Semua</a>
                        <a href="<?= base_url('babikode/point_log?type=plus'); ?>" class="btn btn-glow btn-primary <?= ($q_type == 'plus') ? 'active' : ''; ?>">Penambahan</a>
                        <a href="<?= base_url('babikode/point_log?type=minus'); ?>" class="btn btn-glow btn-primary <?= ($q_type == 'minus') ? 'active' : ''; ?>">Pengurangan</a>
                    </div>
                    <form method="get" class="row mt-2">
                        <?php
                        if (isset($q_type) and !empty($q_type)) { ?>
                            <input type="hidden" name="type" id="q_type" value="<?= $q_type; ?>">
                        <?php
                        } ?>
                        <div class="col-md-3">
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered">
                            ...
                            <tbody>
                                ...
                                $query_list = "SELECT point_logs.*, users.phone FROM point_logs INNER JOIN users ON point_logs.user_id = users.id WHERE point_logs.id <> ''";

                                if (isset($q_type) && in_array($q_type, array('hasil','plus', 'minus'))) {
                                    $query_list .= " AND point_logs.type = '" . $q_type . "'";
                                }

                                if (!empty($q_start_date) and !empty($q_end_date)) {
                                    $query_list .= " AND DATE(point_logs.created_at) BETWEEN '" . $q_start_date . "' AND '" . $q_end_date . "'";
                                } elseif (!empty($q_start_date) and empty($q_end_date)) {
                                    $query_list .= " AND DATE(point_logs.created_at) BETWEEN '" . $q_start_date . "' AND '" . $q_start_date . "'";
                                } elseif (empty($q_start_date) and !empty($q_end_date)) {
                                    $query_list .= " AND DATE(point_logs.created_at) BETWEEN '" . $q_end_date . "' AND '" . $q_end_date . "'";
                                }

                                if (!empty($q_search)) {
                                    $query_list .= " AND (users.phone LIKE '%$q_search%' OR point_logs.id LIKE '%$q_search%' OR point_logs.amount LIKE '%$q_search%' OR point_logs.description LIKE '%$q_search%')";
                                }

                                $query_list .= " ORDER BY point_logs.id DESC";
                                ...
                                if (mysqli_num_rows($new_query) == 0) {
                                    echo '<tr><td colspan="6" align="center">Data belum tersedia.</td></tr>';
                                }
                                ...
                                while ($log = mysqli_fetch_assoc($new_query)) {
                                    ...
                                    $type = match ($log['type']) {
                                        'minus' => 'Penambahan Saldo',
                                        default => 'Pengurangan Saldo'
                                    };
                                ?>
                                    <tr>
                                        <td><?= $log['id']; ?></td>
    ...
    $(function() {
        $('#table-row').on('change', function() {
            ...
            if (type == '' || type == undefined) {
                window.location = "<?= base_url('babikode/point_log/'); ?>?row=" + row + "&start_date=" + start_date + "&end_date=" + end_date + "&search=" + search;
            } else {
                window.location = "<?= base_url('babikode/point_log/'); ?>?type=" + type + "&row=" + row + "&start_date=" + start_date + "&end_date=" + end_date + "&search=" + search;
            }
        });
    });
